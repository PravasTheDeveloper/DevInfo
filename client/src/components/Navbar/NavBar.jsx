...
function NavBar() {
    ...
    const NotificationBell = useSelector(state => state.notification.length)
            ...
            <div className='h-[6vh] w-full bg-slate-700 flex items-center text-white'>
                <div className='flex justify-between container mx-auto'>
                    ...
                    <div className='flex items-center text-2xl'>
                        ...
                        <div className='flex justify-center relative group ml-10' onClick={() => { setprofileBarToggle(false) }}>
                            <div className='w-10 h-10 bg-slate-500 hover:bg-slate-400 hover:text-slate-700 cursor-pointer duration-300 text-white flex justify-center items-center rounded-full relative' onClick={() => { setNotification(!Notification); setprofileBarToggle(false); setfriendsSection(false) }}><AiOutlineBell />
                                <div className='w-5 h-5 text-base top-[-5px] right-[-10px] bg-rose-600 flex justify-center items-center absolute rounded-full text-white'>
                                    {NotificationBell}
                                </div>
                            </div>
                            <div className='absolute px-3 py-1 group-hover:flex justify-center hidden ease-in duration-500 items-center text-base text-slate-200 bg-zinc-600 bottom-[-50px] shadow-xl rounded z-50'>
                        ...
                        <div className='flex justify-center relative ml-10'>
                            ...
                            < div className={profileBarToggle === false ? 'w-[300px] h-auto absolute bg-slate-100 top-[60px] right-0 rounded-md shadow-xl p-5 hidden' : 'w-[300px] h-auto absolute bg-slate-100 top-[60px] right-0 rounded-md shadow-xl p-5'} >
                                <Link className='w-full h-[60px] bg-white hover:bg-slate-200 rounded-xl shadow-lg flex items-center px-4 cursor-pointer' to={`/account/${UserAuthData._id}`}>
